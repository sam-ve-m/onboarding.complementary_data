...
async def test_when_update_user_success_then_mock_was_called(
    ...
    user_complementary_data = (
        await comp_data_service.complementary_data_model.get_user_update_template()
    )

    mock_validate_cpf.assert_called_once_with()
    mock_audit.assert_called_once_with(
        complementary_data_model=comp_data_service.complementary_data_model
    )
...
@pytest.mark.asyncio
@patch(
    "func.src.services.complementary_data.OnboardingSteps.get_user_current_step",
...
async def test_when_current_step_correct_then_return_true(
    ...
    result = await comp_data_service.validate_current_onboarding_step(jwt="123")
...
async def test_when_current_step_invalid_then_return_raises(
    ...
    with pytest.raises(InvalidOnboardingCurrentStep):
        await comp_data_service.validate_current_onboarding_step(jwt="123")
